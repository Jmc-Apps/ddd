(() => {
  ...
  async function audioEnvelopeForClip(clip){
    if(videoWaveCache.has(clip.id))return videoWaveCache.get(clip.id);const task=(async()=>{if(!clip.sourceFile&&!clip.url)throw new Error(`Reload ${clip.name} first.`);const AudioContextClass=window.AudioContext||window.webkitAudioContext;if(!AudioContextClass)throw new Error('Audio analysis is not supported by this browser.');let data;try{data=clip.sourceFile?await clip.sourceFile.arrayBuffer():await fetch(clip.url).then(response=>{if(!response.ok)throw new Error(`HTTP ${response.status}`);return response.arrayBuffer();});}catch{throw new Error(`The local file ${clip.name} could not be read for audio analysis. Remove it and add it again.`);}const context=new AudioContextClass();try{let buffer;try{buffer=await context.decodeAudioData(data);}catch{throw new Error(`${clip.name} is too large for this browser to decode for audio analysis. Close other tabs and retry, or split the video into shorter clips.`);}const channel=buffer.getChannelData(0),hz=10,step=Math.max(1,Math.floor(buffer.sampleRate/hz)),values=[];for(let start=0;start<channel.length;start+=step){const end=Math.min(channel.length,start+step);let sum=0;for(let index=start;index<end;index++)sum+=channel[index]*channel[index];values.push(Math.sqrt(sum/Math.max(1,end-start)));}return {values,hz,duration:buffer.duration};}finally{data=null;try{await context.close();}catch{}}})();videoWaveCache.set(clip.id,task);try{return await task;}catch(error){videoWaveCache.delete(clip.id);throw error;}
  }
