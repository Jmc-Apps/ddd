(() => {
  ...
  function selectedGoalie(dayId,drillId,lane){
    const key=stationKey(dayId,drillId,lane),other=isMatchSituation(drillId)?state.stationSelections[stationKey(dayId,drillId,lane?0:1)]:'',eligible=eligibleGoaliesForDay(dayId).filter(g=>g.id!==other);
  ...
  function isMatchSituation(drillId){return state.drills.find(d=>d.id===drillId)?.name==='Match Situation'}
  function goalName(lane,drillId){return isMatchSituation(drillId)?`Goal ${lane?'B':'A'}`:'Goal'}
  ...
  function bindStationControls(container,dayId,drillId,live){
    ...
    container.querySelectorAll('.station-card').forEach(card=>{
      ...
      goalieSelect.addEventListener('change',()=>{const shouldAutoStart=Boolean(live&&(activeTimers[lane]||pendingGoalieStarts[lane]));if(live){cancelPendingGoalieStart(lane);stopTimer(lane);}else if(activeVideoTimers[lane])stopVideoGoalieTimer(lane,false);const other=isMatchSituation(drillId)?state.stationSelections[stationKey(dayId,drillId,lane?0:1)]:'';if(other&&goalieSelect.value===other){showToast('The same goalkeeper cannot be placed in Goal A and Goal B.');live?renderRecording():renderVideoReview();return;}state.stationSelections[stationKey(dayId,drillId,lane)]=goalieSelect.value;saveState();if(shouldAutoStart&&goalieSelect.value)scheduleGoalieStart(lane,dayId,drillId,goalieSelect.value);live?renderRecording():renderVideoReview();});
  ...
  function recordOutcome(card,lane,dayId,drillId,live){
    ...
    const goal=isMatchSituation(drillId)?(lane?'B':'A'):'A',event={id:uid(),createdAt:new Date().toISOString(),dayId,drillId,goalieId,lane,goal,outcome:selection.outcome,shotType:selection.shotType,situation:selection.situation,outnumbered:selection.outnumbered,rebound,dangerousRebound:rebound==='Dangerous',note:selection.note||'',source:live?'live':'video',...(live?{}:{videoReview:{masterTime:currentVideoSession().masterTime,syncOffset:currentVideoSession().syncOffset,goal,angleIndex:goal==='B'?1:0}})};
  ...
  function renderEventTimeline(targetId,dayId,drillId,context){
    ...
    $(targetId).innerHTML=events.length?events.map(event=>{const goalie=state.goalies.find(g=>g.id===event.goalieId),goal=isMatchSituation(event.drillId)?(event.goal||(Number(event.lane)===1?'B':'A')):'A',linked=Number.isFinite(Number(event.videoReview?.masterTime)),videoAction=context==='video'?`<button type="button" class="mini-button" data-link-video-shot="${event.id}" ${completedDay(event.dayId)?'disabled':''}>${linked?'Move time':'Set time'}</button>${linked?`<button type="button" class="mini-button" data-export-video-shot="${event.id}">Export Clip</button>`:''}`:'';return `<div class="timeline-event editable ${context==='video'?'video-linked-actions':''}"><span>${new Date(event.createdAt).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}</span><div><strong class="timeline-goalie-name">${goalie?goalieKitIcon(goalie,event.dayId):''}<span>${esc(goalie?.name||'Unknown goalkeeper')}</span></strong><br><span class="muted">${esc(event.shotType)} · ${esc(event.situation)}${event.rebound?` · ${esc(event.rebound)} rebound`:''}${context==='video'?` · ${isMatchSituation(event.drillId)?`Goal ${goal}`:'Goal'}`:''}${linked?` · Video ${formatTime(Number(event.videoReview.masterTime)*1000)}`:''}</span></div><span class="event-outcome ${event.outcome.split(' ')[0]}">${esc(event.outcome)}</span>${videoAction}<button type="button" class="mini-button" data-edit-shot="${event.id}" ${completedDay(event.dayId)?'disabled':''}>Edit</button></div>`}).join(''):'<div class="empty-state">Recorded events will appear here.</div>';
    ...
    $(targetId).querySelectorAll('[data-link-video-shot]').forEach(button=>button.addEventListener('click',()=>{const event=state.events.find(item=>item.id===button.dataset.linkVideoShot);if(!event||completedDay(event.dayId))return;const goal=isMatchSituation(event.drillId)?(event.goal||(Number(event.lane)===1?'B':'A')):'A';event.goal=goal;event.lane=goal==='B'?1:0;event.videoReview={masterTime:currentVideoSession().masterTime,syncOffset:currentVideoSession().syncOffset,goal,angleIndex:goal==='B'?1:0};saveState('Shot linked to video time');renderVideoReview();showToast('Shot placed on the video timeline.');}));
    $(targetId).querySelectorAll('[data-export-video-shot]').forEach(button=>button.addEventListener('click',()=>exportTrialShotClip(button.dataset.exportVideoShot)));
  }
  ...
  function saveShotEdit(formEvent){
    ...
    const drill=state.drills.find(item=>item.id===event.drillId),locked=sectionSituation(drill),rebound=$('editShotRebound').value,goal=drill?.name==='Match Situation'&&$('editShotGoal').value==='B'?'B':'A';Object.assign(event,{goalieId:$('editShotGoalie').value,goal,lane:goal==='B'?1:0,outcome:$('editShotOutcome').value,shotType:$('editShotType').value,situation:locked||$('editShotSituation').value,outnumbered:$('editShotOutnumbered').value,rebound:rebound==='No rebound'?'':rebound,dangerousRebound:rebound==='Dangerous',note:$('editShotNote').value.trim(),updatedAt:new Date().toISOString()});if(event.videoReview){event.videoReview.goal=goal;event.videoReview.angleIndex=goal==='B'?1:0;}saveState('Shot updated');$('shotEditorDialog').close();editingEventContext==='video'?renderVideoReview():renderRecording();showToast('Recorded shot updated.');
  }
  ...
  function currentVideoSession(){
    const key=videoSessionKey();if(!videoReviewSessions.has(key))videoReviewSessions.set(key,{angles:[newVideoAngle(),newVideoAngle()],masterTime:0,syncOffset:0,speed:1,playing:false,autoSuggestion:null,syncMarkers:{A:null,B:null},waveformsVisible:false,syncCollapsed:false,loop:null,restoreAttempted:false});const session=videoReviewSessions.get(key);session.syncMarkers=session.syncMarkers||{A:null,B:null};session.loop=session.loop||null;session.syncCollapsed=Boolean(session.syncCollapsed);session.angles.forEach(angle=>angle.view=angle.view||{zoom:1,x:0,y:0});return session;
  }
  ...
  function resetVideoView(index){currentVideoSession().angles[index].view={zoom:1,x:0,y:0};applyVideoView(index);saveVideoView(index);showToast(`${goalName(index,$('videoDrill')?.value)} framing reset.`);}
  function videoLaneCount(){return isMatchSituation($('videoDrill')?.value)?2:1;}
  ...
  function masterDuration(session){
    const a=playlistDuration(session.angles[0]);if(videoLaneCount()===1)return a;const b=playlistDuration(session.angles[1]);if(a&&b)return Math.max(0,Math.min(a,b-session.syncOffset));return a||Math.max(0,b-session.syncOffset)||0;
  }
  ...
  function syncBothVideos(force=false){
    if(videoControllerBusy)return;videoControllerBusy=true;const session=currentVideoSession(),indices=videoLaneCount()===2?[0,1]:[0];if(indices.length===1){$('videoB').pause();}indices.forEach(index=>{const video=$(index?'videoB':'videoA'),target=angleTargetTime(session,index),actual=currentAnglePlaylistTime(index);if(force||Math.abs(actual-target)>.12)loadVideoAngle(index,target,session.playing);else{video.playbackRate=session.speed;if(session.playing&&video.paused)video.play().catch(()=>{});}});videoControllerBusy=false;
  }
  ...
  function syncCurrentVideoFrames(){
    const session=currentVideoSession();if(!session.angles.every(angle=>angle.clips.length)){showToast('Add videos to both goals before syncing.');return;}stopVideoPlayback();const a=currentAnglePlaylistTime(0),b=currentAnglePlaylistTime(1);session.syncOffset=b-a;session.masterTime=a;session.syncMarkers={A:a,B:b};session.autoSuggestion=null;renderAutoSyncPanel();renderVideoWaveforms();$('videoSyncStatus').textContent=`Goals synced at A ${formatTime(a*1000)} and B ${formatTime(b*1000)} · offset ${session.syncOffset>=0?'+':''}${session.syncOffset.toFixed(2)}s`;setVideoMasterTime(a,true);showToast('Both goal videos are synced.');
  }
  function clearVideoSync(){const session=currentVideoSession();stopVideoPlayback();session.syncOffset=0;session.autoSuggestion=null;session.syncMarkers={A:null,B:null};session.loop=null;renderAutoSyncPanel();renderVideoWaveforms();$('videoSyncStatus').textContent='Goals start together. Set both to the same visible moment and choose Sync current frames for a precise offset.';setVideoMasterTime(session.masterTime,true);}
  function nudgeVideoAngleB(seconds){const session=currentVideoSession(),angle=session.angles[1];if(!angle.clips.length){showToast('Add videos to Goal B first.');return;}stopVideoPlayback();loadVideoAngle(1,currentAnglePlaylistTime(1)+Number(seconds||0),false);}
  ...
  function placeVideoSyncMarker(index){const session=currentVideoSession(),key=index?'B':'A',angle=session.angles[index];if(!angle.clips.length){showToast(`Add videos to Goal ${key} first.`);return;}stopVideoPlayback();session.syncMarkers[key]=currentAnglePlaylistTime(index);updateVideoTransport();renderVideoWaveforms();$('videoSyncStatus').textContent=`Marker ${key} placed at ${formatTime(session.syncMarkers[key]*1000)}${[session.syncMarkers.A,session.syncMarkers.B].every(Number.isFinite)?' · Sync Markers is ready.':''}`;}
  function applyVideoSyncMarkers(){const session=currentVideoSession(),a=session.syncMarkers.A,b=session.syncMarkers.B;if(!Number.isFinite(a)||!Number.isFinite(b)){showToast('Place a marker on both goal waveforms first.');return;}session.syncOffset=b-a;session.masterTime=a;session.autoSuggestion=null;renderAutoSyncPanel();setVideoMasterTime(a,true);renderVideoWaveforms();$('videoSyncStatus').textContent=`Markers synced · A ${formatTime(a*1000)} · B ${formatTime(b*1000)} · offset ${session.syncOffset>=0?'+':''}${session.syncOffset.toFixed(2)}s`;showToast('Both goal playlists synced to the manual markers.');}
  ...
  function waveformBounds(session){const durationA=playlistDuration(session.angles[0]),durationB=playlistDuration(session.angles[1]),min=Math.min(0,-Number(session.syncOffset||0)),max=Math.max(durationA,durationB-Number(session.syncOffset||0));return{min,max,span:Math.max(.01,max-min)};}
  function drawVideoWaveform(canvas,envelope,index){const session=currentVideoSession(),bounds=waveformBounds(session),context=canvas.getContext('2d'),width=canvas.width=1200,height=canvas.height=76,mid=height/2,offset=index?-session.syncOffset:0,startX=(offset-bounds.min)/bounds.span*width,drawWidth=playlistDuration(session.angles[index])/bounds.span*width;context.fillStyle='#06111a';context.fillRect(0,0,width,height);context.fillStyle=index?'#ef8d2f':'#36b7d9';for(let x=0;x<Math.max(1,Math.floor(drawWidth));x++){const sample=Math.min(envelope.env.length-1,Math.floor(x/Math.max(1,drawWidth)*envelope.env.length)),amplitude=Math.min(mid-4,Math.abs(envelope.env[sample])*(mid*.36)+2);context.fillRect(Math.floor(startX+x),mid-amplitude,1,amplitude*2);}const marker=session.syncMarkers[index?'B':'A'];if(Number.isFinite(marker)){const masterMarker=marker-(index?session.syncOffset:0),x=Math.max(0,Math.min(width,(masterMarker-bounds.min)/bounds.span*width));context.strokeStyle='#fff';context.lineWidth=3;context.beginPath();context.moveTo(x,0);context.lineTo(x,height);context.stroke();context.fillStyle='#fff';context.beginPath();context.arc(x,8,6,0,Math.PI*2);context.fill();}}
  async function renderVideoWaveforms(){const session=currentVideoSession(),panel=$('videoWaveformPanel');if(!session.waveformsVisible||videoLaneCount()!==2){panel.classList.add('hidden');panel.innerHTML='';return;}panel.classList.remove('hidden');const key=videoSessionKey();panel.innerHTML='<div class="video-waveform-intro"><strong>Manual sync waveforms</strong><br>Click the matching sound on each waveform to place Marker A and Marker B, then choose Sync Markers. Synced waveforms share one visual timeline.</div><div class="video-waveform-loading">Generating audio waveforms locally… Longer files may take a few minutes.</div>';try{const results=[];for(const angle of session.angles)results.push(angle.clips.length?await playlistAudioEnvelope(angle):null);if(key!==videoSessionKey())return;panel.innerHTML=results.map((result,index)=>{const marker=session.syncMarkers[index?'B':'A'];return `<div class="video-waveform-row"><div class="video-waveform-label"><strong>Goal ${index?'B':'A'}</strong><span>${Number.isFinite(marker)?`Marker ${formatTime(marker*1000)}`:'No marker placed'}</span></div>${result?`<canvas id="videoWaveform${index}" aria-label="Goal ${index?'B':'A'} audio waveform"></canvas>`:'<div class="muted">Add videos to this goal.</div>'}</div>`}).join('');results.forEach((result,index)=>{if(!result)return;const canvas=$(`videoWaveform${index}`);drawVideoWaveform(canvas,result,index);canvas.addEventListener('pointerdown',event=>{const active=currentVideoSession(),keyName=index?'B':'A',rect=canvas.getBoundingClientRect(),bounds=waveformBounds(active),master=bounds.min+Math.max(0,Math.min(1,(event.clientX-rect.left)/rect.width))*bounds.span,time=Math.max(0,Math.min(playlistDuration(active.angles[index]),master+(index?active.syncOffset:0)));stopVideoPlayback();active.syncMarkers[keyName]=time;loadVideoAngle(index,time,false);updateVideoTransport();renderVideoWaveforms();$('videoSyncStatus').textContent=`Marker ${keyName} placed at ${formatTime(time*1000)}${[active.syncMarkers.A,active.syncMarkers.B].every(Number.isFinite)?' · Sync Markers is ready.':''}`;});});}catch(error){if(key!==videoSessionKey())return;panel.innerHTML=`<strong>Waveforms could not be generated.</strong><div class="video-waveform-intro">${esc(error.message||String(error))} Videos without usable audio can still be aligned with the frame and one-second controls.</div>`;}}
  ...
  function renderAutoSyncPanel(){const session=currentVideoSession(),panel=$('videoAutoSyncPanel'),suggestion=session.autoSuggestion;if(!suggestion){panel.classList.add('hidden');panel.innerHTML='';return;}panel.classList.remove('hidden');if(suggestion.status==='analysing'){panel.innerHTML='<strong>Analysing both goal playlist audio tracks…</strong><span>This stays on this device and may take a moment.</span>';return;}if(suggestion.status==='applied'){panel.innerHTML=`<strong>Auto Sync applied</strong><span>Goal B offset ${suggestion.offset>=0?'+':''}${suggestion.offset.toFixed(2)}s. Play both goals to verify the result.</span>`;return;}const low=suggestion.confidence<60;panel.innerHTML=`<strong>Auto Sync suggestion</strong><span>Audio waveform correlation compared the complete Goal A and Goal B playlists.</span><div class="auto-sync-result"><div><span class="auto-sync-value">Goal B ${suggestion.offset>=0?'+':''}${suggestion.offset.toFixed(2)}s</span><span class="auto-sync-confidence ${low?'low':''}">${suggestion.confidence}% confidence</span></div><span>${low?'Low confidence — review carefully or use manual alignment.':'Review the proposed offset before accepting it.'}</span></div><button type="button" class="button primary" data-accept-auto-sync>Accept Auto Sync</button>`;}
  async function suggestAutoSync(){const session=currentVideoSession();if(!session.angles.every(angle=>angle.clips.length)){showToast('Add videos to both goal playlists first.');return;}stopVideoPlayback();session.autoSuggestion={status:'analysing'};renderAutoSyncPanel();$('videoSuggestAutoSync').disabled=true;try{const a=await playlistAudioEnvelope(session.angles[0]),b=await playlistAudioEnvelope(session.angles[1]),suggestion=findAutoSyncOffset(a.env,b.env,a.hz);if(suggestion.score<=-1)throw new Error('The audio tracks did not have enough overlap to compare.');session.autoSuggestion={...suggestion,status:'suggested'};renderAutoSyncPanel();}catch(error){session.autoSuggestion=null;const panel=$('videoAutoSyncPanel');panel.classList.remove('hidden');panel.innerHTML=`<strong>Auto Sync could not analyse these videos.</strong><span>${esc(error.message||String(error))} Use the manual Goal B alignment controls instead.</span>`;}finally{$('videoSuggestAutoSync').disabled=false;}}
  function acceptAutoSync(){const session=currentVideoSession(),suggestion=session.autoSuggestion;if(!suggestion||suggestion.status!=='suggested')return;session.syncOffset=suggestion.offset;session.autoSuggestion={...suggestion,status:'applied'};$('videoSyncStatus').textContent=`Auto synced · Goal B offset ${session.syncOffset>=0?'+':''}${session.syncOffset.toFixed(2)}s`;setVideoMasterTime(session.masterTime,true);renderVideoWaveforms();renderAutoSyncPanel();showToast('Auto Sync applied to both goal playlists.');}
  ...
  function safeVideoFilename(value){return String(value||'clip').trim().replace(/[^a-z0-9._-]+/gi,'_').replace(/^_+|_+$/g,'').slice(0,100)||'clip';}
  function drawExportGoalieIcon(context,goalie,dayId,x,y,size){const smock=SMOCK_COLOURS[goalie?.smocks?.[dayId]||'White'],pads=SMOCK_COLOURS[goalie?.legGuards?.[dayId]||'White'],scale=size/58;context.save();context.translate(x,y);context.scale(scale,scale);context.lineWidth=2;context.strokeStyle='#092238';context.fillStyle='#dce4ea';context.beginPath();context.arc(24,8,6,0,Math.PI*2);context.fill();context.stroke();context.fillStyle=smock;context.beginPath();context.moveTo(15,16);context.lineTo(24,12);context.lineTo(33,16);context.lineTo(36,33);context.lineTo(12,33);context.closePath();context.fill();context.stroke();context.fillStyle=pads;context.fillRect(14,32,20,10);context.fillRect(13,40,9,15);context.fillRect(26,40,9,15);context.strokeRect(14,32,20,10);context.strokeRect(13,40,9,15);context.strokeRect(26,40,9,15);context.restore();}
  function drawTrialClipOverlay(context,event,shotNumber,width,height){const goalie=state.goalies.find(item=>item.id===event.goalieId),day=state.days.find(item=>item.id===event.dayId),drill=state.drills.find(item=>item.id===event.drillId),goal=isMatchSituation(event.drillId)?`Goal ${event.goal||(Number(event.lane)===1?'B':'A')}`:'Goal',lines=[`${state.trial.name||'Goalkeeper Trial'} · ${dayLabel(day?.date)} · ${drill?.name||'Trial section'}`,`${goal} · Shot ${shotNumber} · ${event.outcome}`,`${event.shotType}${event.situation?` · ${event.situation}`:''}${event.outnumbered&&event.outnumbered!=='Not Out Numbered'?` · ${event.outnumbered}`:''}`,`${event.rebound?`${event.rebound} rebound`:''}${event.note?`${event.rebound?' · ':''}${event.note}`:''}`].filter(Boolean);const pad=Math.max(18,width*.018),line=Math.max(24,width*.024),icon=Math.max(54,width*.055),boxHeight=pad*2+Math.max(icon,lines.length*line),top=height-boxHeight-pad;context.save();context.fillStyle='rgba(3,20,34,.84)';context.fillRect(0,top,width,boxHeight+pad);drawExportGoalieIcon(context,goalie,event.dayId,pad,top+pad,icon);context.fillStyle='#fff';context.font=`700 ${Math.max(22,width*.025)}px system-ui,sans-serif`;context.fillText(goalie?.name||'Goalkeeper',pad+icon+pad,top+pad+line*.8);context.font=`500 ${Math.max(16,width*.016)}px system-ui,sans-serif`;lines.forEach((text,index)=>context.fillText(text,pad+icon+pad,top+pad+line*(index+1.75)));context.restore();}
  function waitForVideoEvent(video,event){return new Promise((resolve,reject)=>{const done=()=>{cleanup();resolve();},fail=()=>{cleanup();reject(new Error('The selected goal video could not be read.'));},cleanup=()=>{video.removeEventListener(event,done);video.removeEventListener('error',fail);};video.addEventListener(event,done,{once:true});video.addEventListener('error',fail,{once:true});});}
  function trialRecorderMime(){return ['video/mp4;codecs=h264,aac','video/mp4','video/webm;codecs=vp9,opus','video/webm'].find(type=>window.MediaRecorder?.isTypeSupported?.(type))||'';}
  async function exportTrialShotClip(eventId){
    if(videoExporting){showToast('A trial clip is already exporting.');return;}const event=state.events.find(item=>item.id===eventId);if(!event||!Number.isFinite(Number(event.videoReview?.masterTime)))return;const session=currentVideoSession(),dual=isMatchSituation(event.drillId);let index=dual?Number(event.videoReview?.angleIndex??((event.goal||(Number(event.lane)===1?'B':'A'))==='B'?1:0)):0,angle=session.angles[index];if(!angle?.clips.length){const other=index?0:1;if(dual&&session.angles[other]?.clips.length&&confirm(`${goalName(index,event.drillId)} has no video. Export this shot from ${goalName(other,event.drillId)} instead?`)){index=other;angle=session.angles[index];}else{showToast(`${goalName(index,event.drillId)} video is not available for this shot.`);return;}}
    const before=Math.max(0,Math.min(60,Number($('videoExportBefore').value)||0)),after=Math.max(1,Math.min(60,Number($('videoExportAfter').value)||8)),master=Number(event.videoReview.masterTime),localShot=master+(index?session.syncOffset:0),localStart=Math.max(0,localShot-before),localEnd=Math.min(playlistDuration(angle),localShot+after),startLocation=locateVideoTime(angle,localStart),endLocation=locateVideoTime(angle,Math.max(localStart,localEnd-.001));if(!startLocation||!endLocation){showToast('The linked shot is outside the available goal video.');return;}if(startLocation.index!==endLocation.index){showToast('This clip crosses two playlist files. Reduce the seconds before or after the shot.');return;}if(!window.MediaRecorder||!HTMLCanvasElement.prototype.captureStream){showToast('Clip export is not supported by this browser.');return;}
    const video=$(index?'videoB':'videoA'),clip=angle.clips[startLocation.index],goalie=state.goalies.find(item=>item.id===event.goalieId),events=getEvents({dayId:event.dayId,drillId:event.drillId}),shotNumber=Math.max(1,events.findIndex(item=>item.id===event.id)+1),previousMaster=session.masterTime;videoExporting=true;stopVideoPlayback();$('videoExportStatus').textContent=`Preparing ${goalName(index,event.drillId)} clip for ${goalie?.name||'goalkeeper'}…`;
    try{if(video.dataset.clipId!==clip.id){video.dataset.clipId=clip.id;video.src=clip.url;video.load();await waitForVideoEvent(video,'loadedmetadata');}video.currentTime=startLocation.time;await waitForVideoEvent(video,'seeked');const width=video.videoWidth||1280,height=video.videoHeight||720,canvas=document.createElement('canvas'),context=canvas.getContext('2d');canvas.width=width;canvas.height=height;const stream=canvas.captureStream(30),mediaStream=video.captureStream?.()||video.mozCaptureStream?.();mediaStream?.getAudioTracks().forEach(track=>stream.addTrack(track));const mimeType=trialRecorderMime(),recorder=new MediaRecorder(stream,mimeType?{mimeType,videoBitsPerSecond:8000000}:undefined),chunks=[];recorder.ondataavailable=chunk=>{if(chunk.data?.size)chunks.push(chunk.data);};const stopped=new Promise((resolve,reject)=>{recorder.onstop=resolve;recorder.onerror=()=>reject(recorder.error||new Error('Clip recording failed.'));});const view=angle.view||{zoom:1,x:0,y:0},draw=()=>{context.fillStyle='#000';context.fillRect(0,0,width,height);context.save();context.translate(width/2+(Number(view.x)||0)*width/Math.max(1,video.clientWidth),height/2+(Number(view.y)||0)*height/Math.max(1,video.clientHeight));context.scale(Number(view.zoom)||1,Number(view.zoom)||1);context.drawImage(video,-width/2,-height/2,width,height);context.restore();drawTrialClipOverlay(context,event,shotNumber,width,height);if(videoExporting&&video.currentTime<endLocation.time&&!video.ended)requestAnimationFrame(draw);};recorder.start(1000);video.playbackRate=1;await video.play();draw();await new Promise(resolve=>{const timer=setInterval(()=>{const progress=Math.max(0,Math.min(1,(video.currentTime-startLocation.time)/Math.max(.01,endLocation.time-startLocation.time)));$('videoExportStatus').textContent=`Exporting ${goalName(index,event.drillId)} clip · ${Math.round(progress*100)}%`;if(video.currentTime>=endLocation.time||video.ended){clearInterval(timer);video.pause();resolve();}},50);});recorder.stop();await stopped;stream.getTracks().forEach(track=>track.stop());const blob=new Blob(chunks,{type:mimeType||'video/webm'}),url=URL.createObjectURL(blob),link=document.createElement('a'),extension=(mimeType||'').includes('mp4')?'mp4':'webm';link.href=url;link.download=`DDD_Trials_${safeVideoFilename(state.trial.name)}_${safeVideoFilename(goalie?.name)}_Shot_${String(shotNumber).padStart(3,'0')}_${safeVideoFilename(goalName(index,event.drillId))}.${extension}`;document.body.appendChild(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),30000);$('videoExportStatus').textContent=`Exported ${goalName(index,event.drillId)} shot ${shotNumber} with overlay and original audio.`;showToast('Trial shot clip exported.');}
    catch(error){$('videoExportStatus').textContent=`Clip export failed: ${error.message||error}`;showToast('The trial clip could not be exported.');}finally{videoExporting=false;setVideoMasterTime(previousMaster,true);}
  }
  ...
  function renderVideoReview(){
    ...
    $('videoLaneB').classList.toggle('hidden',lanes!==2);$('videoLaneA').classList.toggle('single-goal',lanes===1);$('videoGoalHeadingA').textContent=lanes===2?'Goal A':'Goal';$('videoGoalHeadingB').textContent='Goal B';$('videoSyncControls').classList.toggle('hidden',lanes!==2);$('videoToggleSyncPanel').classList.toggle('hidden',lanes!==2);
    ...
    const videoSession=currentVideoSession();$('videoSyncStatus').textContent=videoSession.syncOffset?`Goals synced · offset ${videoSession.syncOffset>=0?'+':''}${videoSession.syncOffset.toFixed(2)}s`:'Goals start together. Use the Goal B alignment buttons, then sync the current frames.';$('videoSyncControls').classList.toggle('collapsed',videoSession.syncCollapsed&&lanes===2);$('videoToggleSyncPanel').textContent=videoSession.syncCollapsed?'Expand Video Sync':'Collapse Video Sync';$('videoToggleSyncPanel').setAttribute('aria-expanded',String(!videoSession.syncCollapsed));$('videoSyncSummary').textContent=lanes===2?`Video Sync · ${videoSession.syncOffset?`offset ${videoSession.syncOffset>=0?'+':''}${videoSession.syncOffset.toFixed(2)}s`:'Goals start together'}`:'Single Goal video';
  ...
  function bindEvents(){
    ...
    document.querySelectorAll('[data-edit-video-playlist]').forEach(button=>button.addEventListener('click',()=>openVideoPlaylist(button.dataset.editVideoPlaylist)));document.querySelectorAll('[data-close-video-playlist]').forEach(button=>button.addEventListener('click',closeVideoPlaylist));$('videoPlaylistAdd').addEventListener('click',()=>chooseVideoFiles(videoPlaylistEditIndex));$('videoPlaylistDialog').addEventListener('cancel',event=>{event.preventDefault();closeVideoPlaylist();});$('videoPlaylistDialog').addEventListener('click',event=>{const move=event.target.closest('[data-video-move]'),remove=event.target.closest('[data-video-remove]');if(move){const [goal,index,direction]=move.dataset.videoMove.split('|').map(Number);moveVideoClip(goal,index,direction);}else if(remove){const [goal,id]=remove.dataset.videoRemove.split('|');removeVideoClip(Number(goal),id);}});
    ...
    $('videoToggleSyncPanel').addEventListener('click',()=>{const session=currentVideoSession();session.syncCollapsed=!session.syncCollapsed;$('videoSyncControls').classList.toggle('collapsed',session.syncCollapsed);$('videoToggleSyncPanel').textContent=session.syncCollapsed?'Expand Video Sync':'Collapse Video Sync';$('videoToggleSyncPanel').setAttribute('aria-expanded',String(!session.syncCollapsed));});
  ...
  function init(){
    ...
    let migratedGoals=false;state.events.forEach(event=>{const goal=isMatchSituation(event.drillId)?(event.goal||(Number(event.lane)===1?'B':'A')):'A';if(event.goal!==goal||Number(event.lane)!==(goal==='B'?1:0)){event.goal=goal;event.lane=goal==='B'?1:0;migratedGoals=true;}if(event.videoReview&&(event.videoReview.goal!==goal||Number(event.videoReview.angleIndex)!==(goal==='B'?1:0))){event.videoReview.goal=goal;event.videoReview.angleIndex=goal==='B'?1:0;migratedGoals=true;}});if(migratedGoals)saveState('Trial shots migrated to goal-based video review');
