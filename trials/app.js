(() => {
  ...
  function currentVideoSession(){
    const key=videoSessionKey();if(!videoReviewSessions.has(key))videoReviewSessions.set(key,{angles:[newVideoAngle(),newVideoAngle()],masterTime:0,syncOffset:0,speed:1,playing:false,autoSuggestion:null,syncMarkers:{A:null,B:null},waveformsVisible:false,loop:null});const session=videoReviewSessions.get(key);session.syncMarkers=session.syncMarkers||{A:null,B:null};session.loop=session.loop||null;return session;
  }
  ...
  async function audioEnvelopeForClip(clip){
    if(videoWaveCache.has(clip.id))return videoWaveCache.get(clip.id);const task=(async()=>{if(!clip.url)throw new Error(`Reload ${clip.name} first.`);const AudioContextClass=window.AudioContext||window.webkitAudioContext;if(!AudioContextClass)throw new Error('Audio analysis is not supported by this browser.');const data=await fetch(clip.url).then(response=>response.arrayBuffer()),context=new AudioContextClass();try{const buffer=await context.decodeAudioData(data),channel=buffer.getChannelData(0),hz=10,step=Math.max(1,Math.floor(buffer.sampleRate/hz)),values=[];for(let start=0;start<channel.length;start+=step){const end=Math.min(channel.length,start+step);let sum=0;for(let index=start;index<end;index++)sum+=channel[index]*channel[index];values.push(Math.sqrt(sum/Math.max(1,end-start)));}return {values,hz,duration:buffer.duration};}finally{try{await context.close();}catch{}}})();videoWaveCache.set(clip.id,task);try{return await task;}catch(error){videoWaveCache.delete(clip.id);throw error;}
  }
  async function playlistAudioEnvelope(angle){const parts=[];for(const clip of angle.clips)parts.push(await audioEnvelopeForClip(clip));const values=parts.flatMap(part=>part.values);if(values.length<100)throw new Error('At least ten seconds of audio is needed in each angle.');const range=values.reduce((result,value)=>({min:Math.min(result.min,value),max:Math.max(result.max,value)}),{min:Infinity,max:-Infinity});if(range.max-range.min<1e-7)throw new Error('The selected videos contain no usable audio variation.');return {env:normalizeAudioEnvelope(values),hz:parts[0]?.hz||10};}
  ...
  function renderVideoReview(){
    ...
    renderVideoPlaylists();updateVideoTransport();renderVideoShotTimeline();renderAutoSyncPanel();$('videoToggleWaveforms').textContent=currentVideoSession().waveformsVisible?'Hide Waveforms':'Show Waveforms';renderVideoWaveforms();syncBothVideos(true);
  }
